...
class ConversationOutputProcessor:
    ...
    def process_llm_output(
        ...
        advisory = {
            "grounding_refs": list(parsed_output.get("grounding_refs", [])),
            "social_intent": parsed_output.get("social_intent", "none"),
            "follow_through": parsed_output.get("follow_through", {}),
        }
        refs = parsed_output.get("grounding_refs", [])
        follow_through = parsed_output.get("follow_through")
        disagreements = []
        if plan:
            refs = [plan["grounding_ref"]] if plan.get("use_history") else []
            planned_kind = plan.get("follow_through", "none")
            follow_through = ({
                "kind": planned_kind,
                "target": plan.get("counterpart", ""),
                "source_ref": plan.get("grounding_ref", ""),
            } if planned_kind != "none" else {})
            if advisory["grounding_refs"] != refs:
                disagreements.append("grounding_refs")
            if advisory["social_intent"] != plan.get("social_intent", "none"):
                disagreements.append("social_intent")
            parsed_output["grounding_refs"] = refs
            parsed_output["social_intent"] = plan.get("social_intent", "none")
        grounding = self.grounding.validate(
            conversation,
            refs, context, follow_through=follow_through,
        )
        if grounding.valid and plan:
            realization = self.grounding.validate_realization(conversation, plan)
            if not realization.valid:
                grounding = type(grounding)(
                    False, realization.reason, realization.candidate_type,
                    valid_refs=refs, follow_through=grounding.follow_through,
                )
        ...
        return {
            ...
            "grounding": grounding,
            "grounding_metadata_advisory": advisory,
            "grounding_metadata_disagreements": disagreements,
        }
